...
function GeneratePage() {
  ...
  const [mg, setMg] = useState({ songId: "", prompt: "", duration: 15, variations: 2, seed: "", prefix: "gen" });
  const [sa, setSa] = useState({ songId: "", prompt: "", duration: 30, variations: 1, seed: "", prefix: "sa" });
  const jobs = useJobs();
  ...
  const setChip = (chip: string) => {
    if (engine === "musicgen") setMg((s) => ({ ...s, prompt: s.prompt ? `${s.prompt}, ${chip}` : chip }));
  ...
  const dest = (prefix: string, songId?: string) => {
    ...
    return s ? `${s.folderPath}\\generations\\${prefix}-*.wav` : `D:\\Varynt\\Projects\\_out\\${prefix}-*.wav`;
  };
  ...
  return (
    <div className="mx-auto max-w-[1600px] px-6 py-6">
      <header className="mb-4">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Instrumental generation with MusicGen and Stable Audio 3 Small-Music.</p>
      </header>
      ...
      <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
        <Panel className="p-5">
          <Tabs value={engine} onValueChange={(v) => setEngine(v as typeof engine)}>
            ...
            <TabsContent value="musicgen" className="mt-4 space-y-4">
              ...
              />
              <DurationField
                value={mg.duration} min={4} max={30}
              ...
              <PrefixField value={mg.prefix} onChange={(v) => setMg((s) => ({ ...s, prefix: v }))} />
            ...
            <TabsContent value="stable-audio-3-small" className="mt-4 space-y-4">
              ...
              <p className="rounded border border-warn/30 bg-warn/5 px-2 py-1.5 text-[11px] text-warn">
                Text-to-audio only. Audio-to-audio and inpainting are planned once the local bridge exposes them.
              </p>
              <SongPicker
                songs={songs ?? []}
                value={sa.songId}
              ...
              />
              <DurationField
                value={sa.duration} min={1} max={120}
              ...
              <PrefixField value={sa.prefix} onChange={(v) => setSa((s) => ({ ...s, prefix: v }))} />
              <p className="rounded border border-border bg-surface-2 px-2 py-1.5 font-mono text-[11px] text-muted-foreground">
        ...
        <div className="space-y-6">
          <Panel className="p-5">
            <PanelHeader title="Queue" description={`${activeGen.length} generation job${activeGen.length === 1 ? "" : "s"}`} />
...
}: { songs: { id: string; title: string }[]; value: string; onChange: (v: string) => void }) {
  return (
    <div>
      <Label>Song</Label>
      <Select value={value} onValueChange={onChange}>
        <SelectTrigger><SelectValue placeholder="No song (Library only)" /></SelectTrigger>
        <SelectContent>
...
function PromptField({
  value, onChange, onChip,
}: { value: string; onChange: (v: string) => void; onChip: (chip: string) => void }) {
  return (
    <div>
      <Label>Prompt</Label>
...
function DurationField({
  ...
  return (
    <div>
      ...
      <Slider min={min} max={max} step={1} value={[value]} onValueChange={([v]) => onChange(v ?? min)} />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="flex gap-2">
        <Input value={value} onChange={(e) => onChange(e.target.value.replace(/[^0-9]/g, ""))} placeholder="random" />
        <Button
